...
function ActivitySearchInner({ cities, activities: all, initialCityId, initialQ }: ActivitySearchProps) {
  ...
  const openAdd = async (a: ActRow) => {
    ...
    // If the user is coming from a city that belongs to one of their trips, no hint needed — they pick.
  ...
  const submitAdd = async () => {
    ...
    setBusy(true);
    // Find a stop in that trip that covers this city
    ...
    <div className="container">
      <div className="page-head">
        <div>
          ...
          <p className="page-sub">{all.length} experiences — sightseeing, food, adventure and more.</p>
        </div>
      </div>
      ...
      <p className="faint mb-16">{list.length} activities shown</p>
      ...
      <div className="grid grid-2">
        {list.map((a, i) => {
          ...
          return (
            ...
            <div className="card card-pad card-hover" style={{ display: 'flex', gap: 14, height: '100%' }}>
              <div className="act-emoji" style={{ background: `${tm.color}1a`, width: 46, height: 46, fontSize: 22 }}>{a.emoji}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div className="flex items-center gap-8 wrap">
                  ...
                  <span className="faint right">{a.city.emoji} {a.city.name}, {a.city.country}</span>
                </div>
                <p className="muted mt-8" style={{ fontSize: 13.5 }}>{a.description}</p>
                <div className="flex items-center gap-8 mt-8">
                  <span className="faint">⏱ {a.duration}h</span>
                  <span style={{ fontWeight: 800 }}>{fmtMoney(a.cost)}</span>
                  <button className="btn btn-ghost btn-sm right" onClick={() => setQuick(a)}>Quick view</button>
      ...
      {list.length === 0 && (
        <div className="card">
          <div className="empty">
            ...
            <div style={{ fontWeight: 700, color: 'var(--ink)' }}>Nothing matches</div>
            <p className="muted mt-8">Loosen a filter or pick another city.</p>
          </div>
        </div>
      )}
      ...
      {quick && (
        <Modal title={quick.name} onClose={() => setQuick(null)}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 14 }}>
            <div className="act-emoji" style={{ background: `${typeMeta(quick.type).color}1a`, width: 54, height: 54, fontSize: 26 }}>{quick.emoji}</div>
            <div>
          ...
          <p className="muted" style={{ fontSize: 14.5 }}>{quick.description}</p>
          <div className="mt-24 flex" style={{ justifyContent: 'flex-end' }}>
      ...
      {addFor && (
        <Modal title={`Add “${addFor.name}”`} onClose={() => setAddFor(null)}>
          <p className="muted mb-16" style={{ fontSize: 14 }}>
            This activity will be attached to a stop in <b>{addFor.city.name}</b>. If your trip has no {addFor.city.name} stop yet, we’ll add one for you (next week by default).
          </p>
          <div className="field">
          ...
          <div className="flex gap-8" style={{ justifyContent: 'flex-end' }}>
            ...
            <button className="btn btn-primary" onClick={submitAdd} disabled={busy || !selTrip}>{busy ? 'Adding…' : 'Add'}</button>
          </div>
        </Modal>
      )}
