...
const DESTINATIONS = [
  { id: 'gujarat', name: 'Gujarat Heritage Express 🇮🇳', dailyBase: 45, cities: ['Ahmedabad', 'Statue of Unity', 'Rann of Kutch'] },
  { id: 'rajasthan', name: 'Rajasthan Royal Circuit 🏰', dailyBase: 60, cities: ['Jaipur', 'Udaipur', 'Jaisalmer'] },
  { id: 'europe', name: 'Europe Grand Tour 🇪🇺', dailyBase: 140, cities: ['Paris', 'Rome', 'Amsterdam', 'Barcelona'] },
  { id: 'asia', name: 'Tropical South-East Asia 🌴', dailyBase: 55, cities: ['Bangkok', 'Bali', 'Singapore'] },
];
...
export function TravelSimulator() {
  ...
  const stayPerDay = Math.round(dest.dailyBase * 0.9 * tierMultiplier);
  const mealsPerDay = Math.round(dest.dailyBase * 0.4 * tierMultiplier);
  const transitTotal = Math.round(120 * travelers * (dest.cities.length - 1));
  const activitiesTotal = Math.round(25 * days * travelers * paceMultiplier);
    ...
    <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-8 space-y-8 my-12 backdrop-blur-xl relative overflow-hidden">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-50 text-amber-700 font-bold text-xs border border-amber-200">
            <i className="bi bi-cpu-fill"></i> Live Interactive Travel Simulator
          </div>
          <h3 className="text-2xl md:text-3xl font-black font-display text-slate-900">
            Real-Time Cost & Route Simulator
          </h3>
          <p className="text-xs text-slate-500">Adjust parameters to simulate live budget, daily spend & transit metrics</p>
        </div>
        ...
        <div className="bg-gradient-to-r from-teal-900 to-teal-800 text-white p-4 rounded-2xl flex items-center gap-6 shadow-lg">
          <div>
            ...
            <div className="text-3xl font-black font-display text-amber-300">${totalCost.toLocaleString()}</div>
          </div>
          <div className="border-l border-teal-700 pl-6 text-right">
            ...
            <div className="text-lg font-bold text-white">${dailyPerPerson}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">1. Choose Circuit</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-800 uppercase tracking-wider mb-2">Hotel Tier</label>
        ...
        <div className="space-y-4 bg-slate-900 text-white p-6 rounded-2xl flex flex-col justify-between shadow-xl">
          <div className="space-y-4">
            <h4 className="text-base font-bold font-display flex items-center justify-between border-b border-slate-800 pb-3">
              <span>Live Simulation Analytics</span>
              <span className="text-xs text-teal-400 font-mono">REALTIME ENGINE</span>
            </h4>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <div className="flex justify-between font-semibold mb-1">
                  ...
                  <span className="font-mono text-teal-300">${stayTotal.toLocaleString()}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between font-semibold mb-1">
                  <span className="text-slate-300">🍜 Dining & Food</span>
                  <span className="font-mono text-amber-300">${mealsTotal.toLocaleString()}</span>
                </div>
                <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                  <div
              ...
              <div>
                <div className="flex justify-between font-semibold mb-1">
                  <span className="text-slate-300">✈️ Inter-City Transit</span>
                  <span className="font-mono text-cyan-300">${transitTotal.toLocaleString()}</span>
                </div>
              ...
              <div>
                <div className="flex justify-between font-semibold mb-1">
                  ...
                  <span className="font-mono text-emerald-300">${activitiesTotal.toLocaleString()}</span>
                </div>
                <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                  <div
          ...
          <div className="pt-4 border-t border-slate-800">
            <div className="text-xs font-bold text-slate-400 mb-2">Simulated Route Circuit:</div>
